		...
		<div id="content">
			...
			<section id="col2">				
				...
				<?php 
					...
					// 한 페이지에 보여질 수 있는 메모글의 수.
					$scale=5;

					// 총 페이지 수(한페이지에 5($scale) 메모글)
					// (1~5: 1page 6~10: 2page, 11~15 3px;)

					$pageNum = ceil($rowNum/$scale); // floor = 내림 round = 반올림 
					if($pageNum == 0) $pageNum = 1; // 게시글이 없을 때.
					...
					// 현재 페이지에서 보여줄 $scale의 메모글 출력하기..
					// $result에서 특정 row로 커서를 이동해서 읽어오기...
					// 커서를 이동시킬 위치(읽어올 시작위치) 계산하기.
					// (page 1 = 시작번호 0, page2 = 시작번호 5, page3 = 시작번호 10)
					// DB TABLE의 row번호는 0번부터...

					$start = ($page-1)*$scale;

					// $rowNum = 총 메모글 수, $pageNum = 총 page 수, $page = 현재 페이지 번호, $start 현재 페이지에서 읽어들일 row의 시작번호.
					for($i= $start; $i<$start+$scale && $i<$rowNum; $i++){
						// 해당하는 row의 위치로 커서를 이동.
						mysqli_data_seek($result, $i);

						$row = mysqli_fetch_array($result);

						// memo테이블의 한 레코드의 필드값을 얻어오기.
						...
						// 읽어온 값들을 출력.
						// 모양설계를 위해 html문법으로 사용.

						?>
							<!--  이 분리된 공간 사이 html -->
							<!-- memo글에 제목 영역(번호, 닉네임, 작서일) -->
							...
							<!-- 내용영역 -->
							<div id="memo_list_content"><?= $memo_content ?></div>
